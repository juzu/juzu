...
* +org.juzu:juzu-dep-cdi:war+

In the following examples we will show the Maven configuration for a couple of examples, all the configurations
...
With the predefined war, it becomes:

[source,xml,indent=0]
.Maven dependencies using the predefined war
----
...
Here are the dependencies for an EE container, note that there is no need to use a predefined way.
...
[source,xml,indent=0]
.Maven dependencies for CDI in a servlet container using the predefined war
----
