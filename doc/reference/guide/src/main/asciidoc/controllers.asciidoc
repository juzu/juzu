...
* Structured data types using the +@juzu.Mapped+ annotation
...
The Json plugin decodes json entities using the https://github.com/FasterXML/jackson[Jackson] framework. It can
...
@Action
@Route("/post")
public Response.View action(MyObj obj) throws IOException {
  ...
}
----

Mapped java objects must be annotated with the +@juzu.plugin.jackson.Jackson+ annotation so the unmarshaller can recognize them.
