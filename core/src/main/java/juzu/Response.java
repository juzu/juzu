 ...
 * view controller, this kind of response can be created using an {@link juzu.request.ActionContext}, however
 * the the preferred way is to use a controller companion class that carries method factories for creating render
 * responses.</p>
 *
 * <p>Type safe {@link juzu.Response.Update} factory method are generated for each view or resource controller methods.
 ...
 * <h2>Mime response</h2>
 * 
 * <p>Mime response are used by the {@link juzu.request.Phase#RENDER} and the {@link juzu.request.Phase#RESOURCE} phases.
 * Both contains a content to be streamed to the client but still they have some noticeable differences.</p>
 * 
 * <p>The {@link juzu.Response.Content} class is the base response class which will work well for the two phases. However the
 * {@link juzu.request.Phase#RENDER} can specify an optional title and the {@link juzu.request.Phase#RESOURCE} can
